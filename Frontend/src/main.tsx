...
import { ChevronLeft, ChevronRight, Download, File as FileIcon, FileImage, FileText, Film, Folder, FolderPlus, LogOut, Maximize, Minimize, Pause, Pencil, Play, Trash2, Upload, Users, X } from 'lucide-react'
import { AccountPanel, AdminPanel } from './admin'
import './styles.css'

type Entry = { name: string; path: string; type: 'directory'|'file'|'blocked'; size: number|null; modified: string; mime: string|null }
type Session = { username: string; is_admin: boolean; csrf: string }
...
function App() {
  const [session,setSession]=useState<Session|null>(null), [loading,setLoading]=useState(true), [path,setPath]=useState(''), [items,setItems]=useState<Entry[]>([]), [error,setError]=useState(''), [progress,setProgress]=useState<number|null>(null), [preview,setPreview]=useState<Entry|null>(null), [drag,setDrag]=useState(false), [view,setView]=useState<'files'|'users'|'account'>('files')
  ...
  return <div className="app" onDragOver={e=>{if(view==='files'){e.preventDefault();setDrag(true)}}} onDragLeave={()=>setDrag(false)} onDrop={e=>{if(view==='files'){e.preventDefault();setDrag(false);upload(e.dataTransfer.files)}}}>
    {drag&&<div className="drop"><Upload size={48}/><b>Engedd el a fájlokat</b></div>}
    <header><div className="brand"><div className="brand-mark"><Folder/></div><span>Családi tárhely</span></div><div className="user"><button className={view==='files'?'nav-active':''} onClick={()=>setView('files')}>Fájlok</button>{session.is_admin&&<button className={view==='users'?'nav-active':''} onClick={()=>setView('users')}><Users size={16}/>Felhasználók</button>}<button className={view==='account'?'nav-active':''} onClick={()=>setView('account')}>{session.username}</button><button className="icon-btn" title="Kijelentkezés" onClick={async()=>{try{await api('/api/auth/logout',{method:'POST'},session.csrf)}catch{}setSession(null);setView('files')}}><LogOut/></button></div></header>
    {view==='users' && session.is_admin ? <main className="content"><AdminPanel csrf={session.csrf} currentUser={session.username}/></main> : view==='account' ? <main className="content"><AccountPanel csrf={session.csrf} onChanged={()=>{setSession(null);setView('files')}}/></main> : <main className="content"><div className="toolbar"><nav>{crumbs.map((c,i)=><React.Fragment key={c.path}><button onClick={()=>refresh(c.path)}>{c.name}</button>{i<crumbs.length-1&&<span>/</span>}</React.Fragment>)}</nav><div className="actions"><button className="secondary" onClick={()=>{const n=prompt('Új mappa neve:');if(n)mutate(`/api/folders?name=${encode(n)}`,{method:'POST',body:JSON.stringify({path})})}}><FolderPlus/>Új mappa</button><button onClick={()=>picker.current?.click()}><Upload/>Feltöltés</button><input ref={picker} hidden type="file" multiple onChange={e=>e.target.files&&upload(e.target.files)}/></div></div>
